...
class LogUtil:
    ...
    def clear_logs(logger: str = "") -> None:
        ...
        # clear all logs
        [
            LogUtil._clear_folder(os.path.join(logs_path, log), ".log")
            for log in os.listdir(logs_path)
            if os.path.isdir(os.path.join(logs_path, log))
        ]
    ...
    def _clear_folder(path: Union[str, Path], extension: str) -> None:
        ...
        files = [
            ...
            and os.path.splitext(x)[1] == extension
        ]
    ...
    def create_log_file(path: str) -> Path:
        ...
        """
        log = Path(
            "{path}{filename}.log".format(
                path=path, filename=datetime.now().strftime("%d.%m.%Y - %H.%M.%S")
            )
        ).absolute()
